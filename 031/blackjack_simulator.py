...
def simulate(queue, batch_size):
    ...
    def play_hand():
        ...
        # TODO: Handle if player or dealer gets two aces, logic currently
        #           has the hand as busting with a 22 (11+11)
        if sum(dealer_cards) == 22 and len(dealer_cards) == 2: # AA hand
            if dealer_cards[0] == 11:
                dealer_cards[0] = 1
        if sum(player_cards) == 22 and len(player_cards) == 2: # AA hand
        ...
        while sum(dealer_cards) < 18:
            ...
            #deal new card
            dealer_cards.append(deck.pop())
            new_card_value = sum(dealer_cards) - hand_value

            #check if ace and if bust, then adjust ace value
            if sum(dealer_cards) > 21 and new_card_value == 11:
        ...
        # check for dealer BJ
        if d_sum == 21 and len(dealer_cards) == 2:
            if p_sum == 21 and len(player_cards) == 2: 
                return 0; # dealer and player has bj - push
            else:
                return -1; # dealer has blackjack
        
        # check for player BJ
        if p_sum == 21 and len(player_cards) == 2:
            if p_sum == 21 and len(dealer_cards) == 2: 
                return 0; # dealer and player has bj - push
            else:
                return 1.5; # player has blackjack
...
if __name__ == '__main__':
    ...
    print("Overall winnings: {} (edge = {} %)".format("{0:.2f}".format(win-lose), "{0:.3f}".format(100.0 * (win-lose) / simulations )))
